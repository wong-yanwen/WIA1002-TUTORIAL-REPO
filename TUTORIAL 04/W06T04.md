...
```
FIXED ANSWER: 
- if the list is empty , firstNode beccomes both head and tail
- if the list is not empty , point firstNode.next to the old head , then update head.
```
...
FIXED ANSWER:
1. create firstNode
2. if head==null → head = tail = firstNode
3. else firstNode.next = head
4. head = firstNode
```
...
Extra info for contains(E e):
Traversal logic:
1. Start at head
2. Compare current.element with e
3. If equal → return true
4. Else move to current.next
...
Extra info for removeLast():
Traversal logic:
1. Move pointer1 until it reaches 
the node just before tail.
2. Store the old tail in temp.
3. Make pointer1 the new tail.
4. Set tail.next = null to detach the 
last node
