...
3. What is a raw type? Why is a raw type unsafe? Why is the raw type allowed in Java?
    ...
    Raw type is unsafe because it can cause unchecked runtime error, especially the ones related to type safety. For example , if we are comparing object1 , which is a String , with object2 ,which is an Integer, it will cause runtime error. 
...
4. What is erasure? Why are Java generics implements using erasure?
   ...
   Erasure prevents some means of violating parametricity, thus eliminating possibilities of incorrect programs , and backward compatibility.
